function pageLayout({ title, content }) {
  ...
  <body>
    <header >
      <h1  ><a class="ey" href="/">Pokedex!</a></h1>
    </header>
...
function frontPage() {
  return `
  <section class="hola">
    <form class="klk"action="/search" method="GET">
      <fieldset>
        <label class="main" for="query">Busca tu pokemon</label>
        <input type="search" name="query" id="query" /> 
      </fieldset>
      <button class="nose">Buscar</button>
    </form>
  </section>
  `;
}
...
function searchResults({ results }) {
  const htmlResults = results.map((result) => {
    ...
    return `<li class="kkk">
              ...
              <a class="papu" href="/pokemon/${result.id}">
                ${english} / ${japanese}
              </a>
              (${result.type.join(", ")})
            </li>`;
